        ...
        <div class="col-lg-12">
          ...
          <div v-if="loggedIn">
              ...
              <div class="row">
                <div class="col-md-6" v-bind:class="{ 'fld-error': $v.form.name.$error }">
                  ...
                  <span class="msg-error" v-if="!$v.form.name.required">
                    <small>Поле необходимо заполнить</small>
                  </span>
                  <span class="msg-error" v-if="!$v.form.name.minLength">
                    <small>Поле должно содержать минимум {{ $v.form.name.$params.minLength.min }} символов.</small>
                  </span>
                </div>
                <div class="col-md-6" v-bind:class="{ 'fld-error': $v.form.email.$error }">
                  ...
                  <span class="msg-error" v-if="!$v.form.email.email">
                    <small>Почта введена некорректно</small>
                  </span>
                </div>
              </div>
              ...
              <div class="row mt-3">
                <div class="col-md-12" v-bind:class="{ 'fld-error': $v.form.subject.$error }">
                  ...
                  <span class="msg-error" v-if="!$v.form.subject.minLength">
                    <small>Поле должно содержать минимум {{ $v.form.subject.$params.minLength.min }} символов.</small>
                  </span>
                </div>
              </div>
            ...
            <div class="text-center text-md-left mt-3">
              <button class="btn btn-primary" type="submit" @click.prevent="submitForm" :disabled='!isComplite'>Отправить</button>
            </div>
          </div>
          <div v-else>
            <h6 class="card-header">Чтобы связаться со мной <nuxt-link to="/signin">авторизуйтесь</nuxt-link> или <nuxt-link to="/signup">зарегистрируйтесь.</nuxt-link></h6>
          </div>
        </div>
      </div>
    </div>
